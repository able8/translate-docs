...
我们将从一个简单的方法开始，确定该方法的挑战和潜在风险，并逐步解决我们在整个系列中确定的每个问题。我们将完成一个配置，该配置利用生命周期挂钩、准备情况探测和 Pod 中断预算来实现我们的零停机时间部署。
...
Drain 操作实现了将所有 Pod 重新调度离开节点的目标。在排放操作期间，节点被标记为不可调度（“NoSchedule”污点）。这可以防止在节点上安排新的 pod。之后，drain 操作开始从节点驱逐 Pod，通过向 Pod 的底层容器发送“TERM”信号来关闭当前在节点上运行的容器。
